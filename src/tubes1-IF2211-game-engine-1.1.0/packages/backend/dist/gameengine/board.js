...
const bot_1 = require("./gameobjects/bot/bot");
class Board {
    config;
    gameObjectProviders;
    logger;
    ...
    callbackLoopsRegistered = {};
    callbackLoopsId = {};
    /** List of callbacks that are triggerred whenever a session is finished. */
    sessionFinishedCallbacks = [];
    botMoves = {};
    botRateLimitViolations = {};
     ...
     * @param callback
     */
    unregisterSessionFinishedCallback(callback) {
        this.sessionFinishedCallbacks = this.sessionFinishedCallbacks.filter((c) => c !== callback);
    }
    /**
     ...
     * @param bot
     */
    createNewExpirationTimer(bot, time = null) {
    ...
    // TODO: Get by id instead? //Klara
    getRateLimitViolations(bot) {
        return this.botRateLimitViolations[bot.name];
    }
    // TODO: Update by id instead? //Klara
    updateRateLimitViolations(bot) {
